  ...
  <div class="app-container">
    <el-card class="filter-container" shadow="never">
      <div>
        ...
        <el-button
          ...
          size="small"
        >{{$t('message.查询搜索')}}</el-button>
        <el-button
          style="float:right;margin-right: 15px"
          @click="handleResetSearch()"
          size="small"
        >{{$t('message.重置')}}</el-button>
      </div>
        ...
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item :label="$t('message.收货人：')">
            <el-input
              ...
              class="input-width"
              :placeholder="$t('message.收货人姓名/手机号码')"
            ></el-input>
          </el-form-item>
    ...
    <el-tabs v-model="activeName" @tab-click="handleTabClick">
      <el-tab-pane :label="$t('message.全部')" name="t1" :key="'t1'">
        <div style="background: rgb(189, 189, 187); display: inline"></div>
      </el-tab-pane>
      <el-tab-pane
        :key="item.label"
        :label="item.label"
      ...
      >
        <div style="background: green; display: inline"></div>
      </el-tab-pane>
    </el-tabs>
    ...
    <div class="table-container">
      <el-table ref="orderTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column :label="$t('message.操作')" min-width="20%" align="center">
          <template slot-scope="scope">
            <div v-if="roles[0]!=$t('message.超级管理员')">
              <el-button-group>
                <el-button size="mini" @click="handleViewOrder(scope.row)" inline>{{$t('message.查看')}}</el-button>

                <el-button
                  size="mini"
                  v-show="scope.row.payStatus === 0 &&  scope.row.payType != 1 "
                  @click="handlePayOrder(scope.$index, scope.row)"
                  inline
                >{{$t('message.确认收款')}}</el-button>

                <el-button
                  size="mini"
                  v-show="scope.row.status=== 0 &&  scope.row.payType == 1 "
                  @click="handleOrderOper($t('message.确认赊账') ,scope.row)"
                  inline
                >{{$t('message.确认赊账')}}</el-button>

                <el-button
                  size="mini"
                  v-show="scope.row.status=== 0 &&  scope.row.payType != 1 "
                  @click="handleOrderOper($t('message.委托收款'), scope.row)"
                  inline
                >{{$t('message.委托收款')}}</el-button>

                <el-button
                  size="mini"
                  v-show="scope.row.status=== 0 &&  scope.row.payType != 1 "
                  @click="handleOrderOper($t('message.中转发货'),scope.row)"
                  inline
                >{{$t('message.中转发货')}}</el-button>
    ...
    <el-dialog :title="OrderOper.title" :visible.sync="OrderOper.dialogVisible" width="30%">
      ...
      <el-input style="width: 80%" :placeholder="$t('message.请输入内容')" v-model="OrderOper.content"></el-input>
    ...
    <el-dialog :title="$t('message.订单支付确认')" :visible.sync="payOrder.dialogVisible" width="30%">
      <span style="vertical-align: top">{{$t('message.操作备注：')}}</span>
      <el-input style="width: 80%" :placeholder="$t('message.请输入内容')" v-model="payOrder.content"></el-input>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      statusOptions: [
        {
          label: this.$t('message.已发货(已收款)'),
          value: 2
        },
        {
          label: this.$t('message.待发货'),
          value: 5
        }
      ],
  ...
  methods: {
    formatStatus(value, pattype) {
      ...
      if (value === 1) {
        ...
        if (pattype == 1) return this.$t('message.已发货(赊账)')
        return this.$t('message.已发货(已收款)')
      } else if (value === 3) {
        return this.$t('message.已发货(已完成)')
      } else if (value === 4) {
